...
def test_bare_package_name_warns(bare_dependency_config: Path, lock_packages: dict) -> None:
...
def test_multiple_hooks_updated(multiple_hooks_config: Path, lock_packages: dict) -> None:
...
def test_bare_name_not_in_missing(bare_unknown_dependency_config: Path, lock_packages: dict) -> None:
...
def test_case_preserved(mixed_case_config: Path, lock_packages: dict) -> None:
    result = update_config(mixed_case_config, lock=lock_packages)
    assert len(result.changes) == 1
    assert "Pydantic==2.13.4" in mixed_case_config.read_text()


def test_compatible_release_pin_updated(compatible_release_config: Path, lock_packages: dict) -> None:
...
def test_lte_pin_updated(less_than_or_equal_config: Path, lock_packages: dict) -> None:
...
def test_ne_pin_updated(not_equal_config: Path, lock_packages: dict) -> None:
    result = update_config(not_equal_config, lock=lock_packages)
    assert len(result.changes) == 1
    assert "pydantic!=2.13.4" in not_equal_config.read_text()


def test_range_collapses_to_exact(range_config: Path, lock_packages: dict) -> None:
...
def test_operator_flag_overrides(exact_config: Path, lock_packages: dict) -> None:
    result = update_config(exact_config, lock=lock_packages, operator="~=")
    assert len(result.changes) == 1
    assert "pydantic~=2.13.4" in exact_config.read_text()


def test_marker_preserved(marker_config: Path, lock_packages: dict) -> None:
...
def test_marker_with_extras_preserved(marker_with_extras_config: Path, lock_packages: dict) -> None:
    result = update_config(marker_with_extras_config, lock=lock_packages)
    assert len(result.changes) == 1
    assert 'pydantic[email]==2.13.4; python_version>="3.11"' in marker_with_extras_config.read_text()


def test_marker_noop_when_current(marker_already_current_config: Path, lock_packages: dict) -> None:
    result = update_config(marker_already_current_config, lock=lock_packages)
    assert not result.changes
    assert not result.warnings


def test_operator_flag_collapses_range(range_config: Path, lock_packages: dict) -> None:
